...
ServiceGateway.prototype.start = function() {
	...
	try {
		...
		setInterval(function() {
			if(!endpoint) {
				// d59ac233 - ZigBee host service with 1 endpoint
				// 936a185c - Echo service with 3 endpoints
				endpoint = self.getEndpointById('936a185c');
			} else {
				if(endpoint.isAlive()) { // Echo service don't have device list, skip by timeout
...
// Looks to the src/dst, sync/async params for routing and requests
ServiceGateway.prototype._messageHandler = function(packet) {
	...
	if(!packet.async) {
		this.syncast(packet).then(function(bunch) {
			debug('syncast', 'response =', bunch);
			srcEp.sendMessage(packet.cmd, bunch);
		});
	} else {
		this.asyncast(packet); // All responses returns to the source
	}
}

// for sync methods
ServiceGateway.prototype.syncast = function(packet, callback) {
	...
	_.forEach(this.endpoints, function(endpoint) {
		if(src != endpoint.getAddress()) {
			(function(ep) {
				endpointsReqs.push(function(rspbunch) {
					// Request with skip proxy
					return ep.request(message.key, packet.data, true).then(function(data) {
						// Data will be null by timeout reject
						rspbunch.push({ dst : ep.getAddress(), data : data || null });
						return rspbunch;
					});
				});
...
ServiceGateway.prototype.asyncast = function(packet) {
}

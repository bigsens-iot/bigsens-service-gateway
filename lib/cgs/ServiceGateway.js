	...
	try {
		...
		setInterval(function() {
			if(!endpoint) {
				...
				// d59ac233 - ZigBee service with 1 endpoint
				// 936a185c - Echo service with 3 endpoints
				endpoint = self.getEndpointById('4f8ed72a');
			} else {
				if(endpoint.isAlive()) {
...
ServiceGateway.prototype.serviceGatewayEndpoint = function() {
	var self = this,
...
ServiceGateway.prototype._packetHandler = function(packet) {
	...
	if(!packet.async) {
		this.syncast(message, packet.data, src, dst).then(function(bunch) {
			var srcEp = this.getEndpointByAddress(src);
			if(srcEp) {
...
ServiceGateway.prototype.syncast = function(message, data, src, dst, callback) {
	...
	_.forEach(this.endpoints, function(endpoint) {
		// TODO: Build reqs based on dst information
		if(src != endpoint.getAddress()) {
			(function(ep) {
				endpointsReqs.push(function(rspbunch) {
					// Request with skip proxy
					return ep.request(message.key, data, true).then(function(rsp) {				
						/* 
						 * FORMAT: Bunch response
						 * {
						 *    dst : 'ip:port',       # service endpoint addr who return response
						 *    data : response        # actual response from endpoint
						 * }
						 * 
						 * NOTE: Response will be undefined by timeout reject
						 */
						rspbunch.push({ dst : ep.getAddress(), data : rsp || null });
						return rspbunch;
					});
				});

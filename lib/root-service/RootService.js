 ...
 * ID       | Endpoint                | Service
 * ---------|-------------------------|----------------------------------
 * 567d9847 | Root Service [server]   | Root Service
 * f74774f5 | Root Service [client]   | Root Service
...
RootService.prototype.attachProxy = function(proxy) {
	var self = this;
 ...
 * 
 * Root Service [server] (ID = 567d9847) <- pipe -> Root Service [client] (ID = f74774f5)
 */
...
RootService.prototype.loopback = function() {
	...
	var sock = net.connect(this.servicePort, function() {
		...
		var root = self.root = new ServiceEndpoint(sock, false, info(false)); 

		var address = root.getAddress();
		log.info('Loopback root service endpoint connected', address);

		// Direction server->client
		root.serviceAnnounce(info(true));

		// We skip endpoints with direction client->server from any *cast(...) calls
		// Finaly loopback the root service endpoint
		self.bindEndpoint(root);
...
RootService.prototype._packetHandler = function(packet) {
	...
	if(message.value == Message.SERVICE_ANNCE // TODO: Message separator
			|| message.value == Message.SERVICE_READY) {

...
function ServiceEndpoint(sock, dir, opts) { // true = server->client, false = client->server
	...
	if(opts) {
		if(opts.name) {
			this.serviceInfo.name = opts.name; 
		}
		if(opts.guid) {
			this.serviceInfo.guid = opts.guid;
		}
		if(this.serviceInfo.name && this.serviceInfo.guid) {
	...
	this.on('message:MESSAGE_REGISTER', this._messageRegister.bind(this));
	this.on('message:MESSAGE_UNREGISTER', this._messageUnregister.bind(this));
}
...
ServiceEndpoint.prototype.isAnnounced = function() {
	return (this.serviceInfo.name && this.serviceInfo.guid);
}

ServiceEndpoint.prototype.isRegistered = function(message) {
	return _.contains(this.messageRegistry, message);
}
...
ServiceEndpoint.prototype.send = function(packet, callback) {
	...
	if(this.isAlive()) {
		...
		this.endpointSocket.on('data', function() {
...
ServiceEndpoint.prototype._messageRegister = function(messages) {
	this.messageRegistry = _.union(this.messageRegistry, messages);
}

ServiceEndpoint.prototype._messageUnregister = function(messages) {
	this.messageRegistry = _.without.apply(_, this.messageRegistry.concat(messages));
	debug('%s -> _messageUnregister => %j', this.getName(), this.messageRegistry);
}
...
ServiceEndpoint.prototype.sendMessage = function(messageName, data, dir, callback) {
	if(!_.isUndefined(dir) && _.isFunction(dir)) { callback = dir; }
	try {
		var deferred = Q.defer(),
			type = Object.prototype.toString.call(messageName)
		...
		this.send(packet).then(function(bytesCount) {
			...
			deferred.resolve(bytesCount);
		}.bind(this)).fail(function(err) { throw err; }).done();
		return deferred.promise.nodeify(callback);
	}
	catch(err) {
